...
function goToSelectedSub(selectedOption) {
 ...
 const link = selectedOption.option || selectedOption.value
...
function download(params) {
 ...
 return new Promise(function (resolve) {
  const url = new URL(params.downloadLink, 'https://www.subdivx.com/');
  const id = url.searchParams.get('id');
  const directory = params.directory ? `/subs/${params.directory}` : '/subs';
  fetch(url.href, {
   headers: {
   ...
   const stream = response.body.pipe(fs.createWriteStream(`${__dirname}${directory}/${id}`));
